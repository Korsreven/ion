...
namespace ion::parallel
{
	...
	///@brief A class representing a pool of workers (with an optional worker id)
	///@details Each worker runs in parallel and returns the results when done.
	...
	class WorkerPool final
	{
		private:
			...
			///@brief Precondition: Caller needs to lock mutex
			void RunQueuedTasks()
			{
		...
		public:
			...
			///@brief Constructs a worker pool, either running (default) or not
			...
			///@brief Constructs a worker pool with the given max number of worker threads, and either running (default) or not
			...
			*/
			...
			///@brief Returns a mutable range of all workers in this manager
			...
			///@brief Returns an immutable range of all workers in this manager
			///@details This can be used directly with a range-based for loop
			...
			*/
			...
			///@brief Sets the max number of worker threads allowed simultaneously, in range [1, ...)
			...
			///@brief Returns all of the result once they are available (blocking)
			///@details Returns the results that are available, then return to caller (non-blocking)
			...
			///@brief Returns the result of the worker with the given id once it is available (blocking)
			///@details Returns the result if available, or return to caller with nullopt (non-blocking)
			...
			///@brief Wait for all of the workers to finish their tasks (blocking)
			void Wait() noexcept
			{
			...
			///@brief Wait for the worker with the given id to finish its task (blocking)
			template <typename = std::enable_if_t<!std::is_same_v<Id, void>>>
			...
			*/
			...
			///@brief Returns true if this is running
			[[nodiscard]] inline auto IsRunning() const noexcept
			{
				return running_state_ == worker_pool::RunningState::NonSuspended;
			}

			///@brief Returns true if one or more workers are active
			[[nodiscard]] inline auto HasActiveWorkers() const noexcept
			{
				return worker_threads_ > 0;
			}

			///@brief Returns the max number of worker threads allowed simultaneously
			...
			*/
			...
			///@brief Clears all workers and all queued work
			void Clear() noexcept
			{
				std::lock_guard lock{m_};
			...
			*/
			...
			///@brief Run or enqueue the given function with the given arguments
			template <typename Function, typename... Args,
			...
			///@brief Run or enqueue the given function with the given arguments
			///@details The task is stored in a map under the given unique id.
			...
			///@brief Suspends all queued work

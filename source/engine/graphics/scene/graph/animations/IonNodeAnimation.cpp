...
{
	//motions_.push_back(detail::rotating_motion{{start_time, 0.0_sec, total_duration},
...
void NodeAnimation::AddScaling(const Vector2 &unit, duration total_duration, duration start_time,
	MotionTechniqueType technique) noexcept
{
	//motions_.push_back(detail::scaling_motion{{start_time, 0.0_sec, total_duration},
	//	{technique, types::Cumulative<real>{unit.X()}}, {technique, types::Cumulative<real>{unit.Y()}}});
}
...
{
	//motions_.push_back(detail::scaling_motion{{start_time, 0.0_sec, total_duration},
	//	{technique_x, types::Cumulative<real>{unit.X()}, {technique_y, types::Cumulative<real>{unit.Y()}}});
}
...
void NodeAnimation::AddTranslation(const Vector2 &unit, duration total_duration, duration start_time,
	MotionTechniqueType technique) noexcept
{
	//motions_.push_back(detail::translating_motion{{start_time, 0.0_sec, total_duration},
	//	{technique, types::Cumulative<real>{unit.X()}, {technique, types::Cumulative<real>{unit.Y()}}});
}

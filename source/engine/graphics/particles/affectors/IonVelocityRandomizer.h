...
namespace ion::graphics::particles::affectors
{
	...
	///@brief A class representing an affector that can add random velocity to single particles
	class VelocityRandomizer final : public Affector
	{
		...
		public:
			...
			///@brief Constructs a new velocity randomizer with the given name
			explicit VelocityRandomizer(std::string name) noexcept;

			///@brief Constructs a new velocity randomizer with the given name, velocity and scope
			VelocityRandomizer(std::string name, real velocity, real scope) noexcept;

			///@brief Constructs a new velocity randomizer with the  given name, min/max velocity and scope
			...
			*/
			...
			///@brief Sets the velocity range of the velocity randomizer to the given range
			...
			///@brief Sets the scope of the velocity randomizer to the given value in range [0.0, 1.0]
			...
			*/
			...
			///@brief Returns the velocity range of the velocity randomizer
			[[nodiscard]] inline auto Velocity() const noexcept
			{
				return velocity_;
			}

			///@brief Returns the scope of the velocity randomizer in range [0.0, 1.0]

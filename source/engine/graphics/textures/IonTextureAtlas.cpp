...
TextureAtlas::TextureAtlas(std::string name, std::string asset_name,
	int rows, int columns, std::optional<int> sub_textures, AtlasSubTextureOrder sub_texture_order) noexcept :

	Texture{std::move(name), std::move(asset_name)},

	rows_{std::clamp(rows, 1, rows)},
	columns_{std::clamp(columns, 1, columns)},
	sub_textures_{std::clamp(sub_textures.value_or(rows * columns), 1, rows * columns)},

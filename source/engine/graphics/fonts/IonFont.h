...
namespace ion::graphics::fonts
{
	...
	///@brief A class representing a font (with a fixed size) that contains multiple glyphs
	///@details Only ASCII and ExtendedASCII character sets are supported for now
	class Font final : public resources::FileResource<FontManager>
	{
		...
		public:
			...
			///@brief Constructs a new font with the given name, asset name, size, face index, character spacing, character set and glyph filter for min/mag
			...
			///@brief Constructs a new font with the given name, asset name, size, character spacing, character set and glyph filter for min/mag
			Font(std::string name, std::string asset_name, int size,
				int character_spacing, font::FontCharacterSet character_set, font::FontGlyphFilter min_filter, font::FontGlyphFilter mag_filter) noexcept;

			///@brief Constructs a new font with the given name, asset name, size, character spacing, character set and glyph filter
			Font(std::string name, std::string asset_name, int size,
				int character_spacing, font::FontCharacterSet character_set, font::FontGlyphFilter filter) noexcept;

			///@brief Constructs a new font with the given name, asset name, size, character spacing and character set
			Font(std::string name, std::string asset_name, int size,
				int character_spacing, font::FontCharacterSet character_set) noexcept;

			///@brief Constructs a new font with the given name, asset name, size and character spacing
			Font(std::string name, std::string asset_name, int size,
			...
			*/
			...
			///@brief Sets the handle for the glyphs to the given handle
			...
			///@brief Sets the glyph data of the font to the given data
			...
			///@brief Sets the glyph metrics of the font to the given metrics
			inline void GlyphMetrics(font::GlyphMetrices glyph_metrics) noexcept
			{
				glyph_metrics_ = std::move(glyph_metrics);
			}

			///@brief Resets the glyph data to save some memory (if not needed anymore)
			...
			*/
			...
			///@brief Returns the handle for each of the glyphs in the font
			///@details Returns nullopt if the font is not loaded
			...
			///@brief Returns the glyph data for the font
			///@details Returns nullopt if the font has not been prepared yet, or is no longer needed (fully loaded or has failed)
			[[nodiscard]] inline auto& GlyphData() const noexcept
			{
				return glyph_data_;
			}

			///@brief Returns the glyph metrics for the font
			///@details Returns nullopt if the font has not been prepared yet
			[[nodiscard]] inline auto& GlyphMetrics() const noexcept
			{
				return glyph_metrics_;
			}

			///@brief Returns the glyph max metrics for the font
			///@details Returns nullopt if the font has not been prepared yet
			...
			///@brief Returns the face index of the font (usually 0)
			[[nodiscard]] inline auto FaceIndex() const noexcept
			{
				return face_index_;
			}

			///@brief Returns the character spacing for the font
			[[nodiscard]] inline auto CharacterSpacing() const noexcept
			{
				return character_spacing_;
			}

			///@brief Returns the character set for the font
			...
			///@brief Returns the min, mag and mip glyph filters for the font
			[[nodiscard]] inline auto GlyphFilter() const noexcept
			{
				return std::pair{glyph_min_filter_, glyph_mag_filter_};
			}

			///@brief Returns the min (minifying) glyph filter for the font
			[[nodiscard]] inline auto GlyphMinFilter() const noexcept
			{
				return glyph_min_filter_;
			}

			///@brief Returns the mag (magnifying) glyph filter for the font

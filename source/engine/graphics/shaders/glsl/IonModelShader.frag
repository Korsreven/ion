...
void main()
{
	...
	if (primitive.has_material)
	{
		...
		{
			vec4 diffuse_map_color = texture(material.diffuse_map, vert_tex_coord);
		...
		{
			vec4 specular_map_color = texture(material.specular_map, vert_tex_coord);

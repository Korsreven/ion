...
/*
	The game class
	--------------
	Should preferably be located in its own h/cpp file
*/

class Game :
	public ion::events::listeners::FrameListener,
	public ion::events::listeners::KeyListener,
	public ion::events::listeners::MouseListener,
	public ion::events::listeners::WindowListener
{
	private:

		ion::NonOwningPtr<ion::gui::GuiController> gui_controller_;
		ion::NonOwningPtr<ion::graphics::scene::graph::SceneGraph> scene_graph_;
		ion::NonOwningPtr<ion::graphics::scene::SceneManager> scene_manager_;
		ion::NonOwningPtr<ion::sounds::SoundManager> sound_manager_;
		ion::NonOwningPtr<ion::graphics::render::Viewport> viewport_;

		ion::NonOwningPtr<ion::graphics::scene::graph::SceneNode> level_node_;
		ion::NonOwningPtr<ion::graphics::scene::graph::SceneNode> splash_node_;
		ion::NonOwningPtr<ion::graphics::scene::graph::SceneNode> player_node_;
		ion::NonOwningPtr<ion::graphics::scene::graph::SceneNode> light_node_;

		ion::NonOwningPtr<ion::graphics::scene::Camera> camera_;
		ion::NonOwningPtr<ion::graphics::scene::Camera> player_camera_;

		ion::NonOwningPtr<ion::sounds::Sound> ambient_rain_;
		ion::NonOwningPtr<ion::graphics::scene::MovableSound> red_lamp_flicker_;
		ion::NonOwningPtr<ion::graphics::scene::MovableSound> green_lamp_flicker_;

		ion::NonOwningPtr<ion::graphics::scene::DrawableText> fps_;
		ion::NonOwningPtr<ion::graphics::scene::Model> pyramid_egyptian_;
		ion::NonOwningPtr<ion::graphics::scene::Model> pyramid_mayan_;
		ion::NonOwningPtr<ion::graphics::scene::graph::animations::NodeAnimationTimeline> player_idle_;

		Vector2 move_player_;
		bool rotate_player_left_ = false;
		bool rotate_player_right_ = false;
		...
		ion::types::Cumulative<duration> query_time_{0.1_sec};

	public:

		//Constructor
		Game(ion::NonOwningPtr<ion::gui::GuiController> gui_controller,
			ion::NonOwningPtr<ion::graphics::scene::graph::SceneGraph> scene_graph,
			ion::NonOwningPtr<ion::graphics::scene::SceneManager> scene_manager,
			ion::NonOwningPtr<ion::sounds::SoundManager> sound_manager,
			ion::NonOwningPtr<ion::graphics::render::Viewport> viewport) :

			gui_controller_{gui_controller},
			scene_graph_{scene_graph},
			scene_manager_{scene_manager},
			sound_manager_{sound_manager},
			viewport_{viewport}
		{
			level_node_ = scene_graph->RootNode().GetChildNode("level_node");
			splash_node_ = scene_graph->RootNode().GetChildNode("splash_node");

			player_node_ = level_node_ ? level_node_->GetDescendantNode("player_node") : nullptr;
			auto ship_node = player_node_ ? player_node_->GetChildNode("ship_node") : nullptr;
			light_node_ = ship_node ? ship_node->GetChildNode("ship_light_node") : nullptr;

			camera_ = scene_manager->GetCamera("main_camera");
			player_camera_ = scene_manager->GetCamera("player_camera");

			ambient_rain_ = sound_manager_->GetSound("ambient_rain");
			red_lamp_flicker_ = scene_manager->GetSound("red_lamp_flicker");
			green_lamp_flicker_ = scene_manager->GetSound("green_lamp_flicker");

			fps_ = scene_manager->GetText("fps");
			pyramid_egyptian_ = scene_manager->GetModel("pyramid_egyptian");
			pyramid_mayan_ = scene_manager->GetModel("pyramid_mayan");
			player_idle_ = ship_node ? ship_node->GetTimeline("ship_idle") : nullptr;
		}
		...
		*/
		...
		bool FrameStarted(duration time) noexcept override
		{
			...
			if (level_node_ && level_node_->Visible())
			{
				...
				if (player_node_)
				{
					if (move_player_ != vector2::Zero ||
						rotate_player_left_ || rotate_player_right_)
					{
						if (move_player_ != vector2::Zero)
							player_node_->Translate(move_player_.NormalizeCopy() * time.count());

						if (rotate_player_left_)
							player_node_->Rotate(math::ToRadians(180.0_r) * time.count());
						if (rotate_player_right_)
							player_node_->Rotate(math::ToRadians(-180.0_r) * time.count());
				...
				if (camera_ && viewport_ &&
					viewport_->ConnectedCamera() == camera_)
				{
					if (move_camera_ != vector2::Zero)
						camera_->ParentNode()->Translate(move_camera_.NormalizeCopy() * time.count());

					if (rotate_camera_left_)
				...
				if (player_camera_ && viewport_ &&
					viewport_->ConnectedCamera() == player_camera_)
				{
					if (rotate_camera_left_)
			...
			if (gui_controller_ && gui_controller_->IsVisible())
				gui_controller_->FrameStarted(time);
		...
		bool FrameEnded(duration time) noexcept override
		{
			...
			if (level_node_ && level_node_->Visible())
			{
				if (query_time_ += time)
				{
					...
					if (!std::empty(result))
					{
						for (auto [a, b] : result)
						{
							if (a == pyramid_egyptian_.get() || b == pyramid_egyptian_.get())
							{
								for (auto &mesh : pyramid_egyptian_->Meshes())
									mesh.BaseColor(color::Orange); //Turn orange
							}
							else if (a == pyramid_mayan_.get() || b == pyramid_mayan_.get())
							{
								for (auto &mesh : pyramid_mayan_->Meshes())
			...
			if (gui_controller_ && gui_controller_->IsVisible())
				gui_controller_->FrameEnded(time);
		...
		*/
		...
		void WindowActionReceived(ion::events::listeners::WindowAction action) noexcept override
		{
			...
			if (gui_controller_ && gui_controller_->IsVisible())
				gui_controller_->WindowActionReceived(action);
		}
		...
		*/
		...
		void KeyPressed(ion::events::listeners::KeyButton button) noexcept override
		{
			//Level
			if (level_node_ && level_node_->Visible() &&
				(!gui_controller_ || !gui_controller_->IsVisible()))
			{
				switch (button)
				{
					//Start moving player forward
					case ion::events::listeners::KeyButton::W:
					move_player_.Y(move_player_.Y() + 1.0_r);
					break;

					//Start moving player left
					case ion::events::listeners::KeyButton::A:
					move_player_.X(move_player_.X() - 1.0_r);
					break;

					//Start moving player backward
			...
			if (gui_controller_ && gui_controller_->IsVisible())
				gui_controller_->KeyPressed(button);
		}

		void KeyReleased(ion::events::listeners::KeyButton button) noexcept override
		{
			//Splash - Press any key to continue
			if (splash_node_ && splash_node_->Visible())
			{	
				splash_node_->Visible(false);

				if (level_node_)
					level_node_->Visible(true);

				if (ambient_rain_)
					ambient_rain_->Play()->Volume(0.2_r);

				if (auto &channel = red_lamp_flicker_->Get(); channel)
					channel->Resume();

				if (auto &channel = green_lamp_flicker_->Get(); channel)
					channel->Resume();
				...
				(!gui_controller_ || !gui_controller_->IsVisible()))
			{
				switch (button)
				{
					...
					*/
					...
					//Stop rotating player CCW
					case ion::events::listeners::KeyButton::Q:
					rotate_player_left_ = false;
					break;

					//Stop rotating player CW
					case ion::events::listeners::KeyButton::E:
					rotate_player_right_ = false;
					break;
					...
					*/
					...
					{
						if (viewport_ && camera_ && player_camera_)
						{
							if (viewport_->ConnectedCamera() == camera_)
								viewport_->ConnectedCamera(player_camera_);
							else if (viewport_->ConnectedCamera() == player_camera_)
								viewport_->ConnectedCamera(camera_);
						}
					...
					{
						if (scene_graph_)
							scene_graph_->FogEnabled(!scene_graph_->FogEnabled());
						break;
					}
					...
					{
						if (light_node_)
							light_node_->Visible(!light_node_->Visible());
						break;
					}
				}
			}
			...
			if (gui_controller_)
			{
				switch (button)
				{
					...
					gui_controller_->Visible(!gui_controller_->IsVisible());
					break;
				}

				//Send key released to GUI
				if (gui_controller_->IsVisible())
					gui_controller_->KeyReleased(button);
			}
  		}
		...
		void CharacterPressed(char character) noexcept override
		{
			...
			if (gui_controller_ && gui_controller_->IsVisible())
				gui_controller_->CharacterPressed(character);
		}
		...
		*/
		...
		void MousePressed(ion::events::listeners::MouseButton button, Vector2 position) noexcept override
		{
			...
			if (gui_controller_ && gui_controller_->IsVisible())
				gui_controller_->MousePressed(button, position);
		}
		...
		void MouseReleased(ion::events::listeners::MouseButton button, Vector2 position) noexcept override
		{
			...
			if (gui_controller_ && gui_controller_->IsVisible())
				gui_controller_->MouseReleased(button, position);
		}
		...
		void MouseMoved(Vector2 position) noexcept override
		{
			...
			if (gui_controller_ && gui_controller_->IsVisible())
				gui_controller_->MouseMoved(position);
		}
		...
		void MouseWheelRolled(int delta, Vector2 position) noexcept override
		{
			...
			if (gui_controller_ && gui_controller_->IsVisible())
				gui_controller_->MouseWheelRolled(delta, position);
		}
};
...
#endif
{
	...
	//GUI
	auto gui_controller = ion::make_owning<ion::gui::GuiController>(scene_graph->RootNode(), viewport);
	...
	else //Initialize programmatically
	{
		...
		*/
		...
		//Lamp flicker
		auto red_lamp_flicker = scene_manager->CreateSound("red_lamp_flicker", lamp_flicker, true);
		auto green_lamp_flicker = scene_manager->CreateSound("green_lamp_flicker", lamp_flicker, true);
		...
		*/
		...
		//Main camera
		auto main_camera_node = scene_graph->RootNode().CreateChildNode({});
		...
		//Player camera
		auto player_camera_node = player_node->CreateChildNode({}, {0.0_r, 0.0_r, 4.55_r});
		player_camera_node->AttachObject(*player_camera);
		...
		*/
		...
		//Ship idle (group)
		auto ship_idle_group = ship_node->CreateAnimationGroup("ship_idle_group");
		ship_idle_group->Add(ship_idle_mover);
		ship_idle_group->Add(ship_idle_rotator);

		//Shop idle (timeline)
		auto ship_idle = ship_node->CreateTimeline("ship_idle", 1.0_r, false);
		ship_idle->Attach(ship_idle_group);
	...
	Game game{gui_controller, scene_graph, scene_manager, sounds, viewport};

	//Subscribe frame and window listeners
	engine.FrameEvents().Subscribe(game);
	window.Events().Subscribe(game);

	//Subscribe key and mouse listeneres
